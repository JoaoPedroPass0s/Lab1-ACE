...
void process_config(){
  ...
  if(config_mode == 0){
    ...
    if(Sgo){
      config.counting_coulour++;
    }
    if(config.counting_coulour > 3){
      config.counting_coulour = 0;
    }
  ...
  if(Sgo){
    ...
    Serial.print("Counting colour :"+config.counting_coulour);
  }
}
...
void pause_timer(const uint32_t currentTime){
  ...
  if(fsm1.pause){
    ...
    for(int i = 0; i <= led; i++){ // Turn on all leds left on the timer
      strip.neoPixelSetValue(i, 50, 50, 50,false);
    }
    strip.neoPixelShow();
...
void loop() 
{
  ...
  if (fsm1.state == sm1_off && Sgo){ // If needed, change the next state
    ...
    if(led < MAXIMUM_NUM_NEOPIXELS - 1){
      led++; 
      strip.neoPixelSetValue(led, 50, 50, 50,true);
    }
  ...
  if (fsm1.state == sm1_off){ 
    ...
    if(led == MAXIMUM_NUM_NEOPIXELS - 1 && fsm1.start){
      strip.neoPixelFill(100,100,100,true);
      fsm1.start = false;
      last_blink = fsm1.tis;
    ...
    if(config.counting_effect == 1 && (fsm1.tis-fsm1.tup >= (intervalOptions[config.timer_time]/2))){
      blink_leds(50,50,50,led,led,50);
    }
  }else if (fsm1.state == sm1_timer && fsm1.pause){
    blink_leds(50,50,50,0,led,250);
